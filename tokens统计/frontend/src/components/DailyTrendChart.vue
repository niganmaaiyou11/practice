  ...
  formatter: (params: any[]) => {
    ...
    for (const p of params) {
      html += `<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
        ...
        <span style="font-weight:600;margin-left:auto;color:${c.tooltipText.value};">${p.value.toLocaleString()}</span>
      </div>`
    }
    return html
  },
...
const chartOption = computed(() => {
  ...
  if (type === 'line') {
    return {
      ...
      grid: { left: 52, right: 24, top: 16, bottom: 60, containLabel: false },
      xAxis: baseXAxis(dates),
      yAxis: baseYAxis.value,
