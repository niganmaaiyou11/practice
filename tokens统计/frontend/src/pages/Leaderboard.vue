  ...
  <div class="leaderboard-page">
    ...
    <section class="table-section">
      ...
      >
        ...
        <el-table-column
          v-if="showMetaColumns"
          :label="t.leaderboard?.priceInput || 'Input $/M'"
          width="130"
          align="right"
        >
          <template #default="{ row }">
            <span v-if="row.price_input != null" class="metric-cell">
              ${{ fmtPrice(row.price_input) }}
            </span>
            <span v-else class="na-cell">—</span>
          </template>
        </el-table-column>
        <el-table-column
          v-if="showMetaColumns"
          :label="t.leaderboard?.priceOutput || 'Output $/M'"
          width="130"
          align="right"
        >
          <template #default="{ row }">
            <span v-if="row.price_output != null" class="metric-cell">
              ${{ fmtPrice(row.price_output) }}
            </span>
            <span v-else class="na-cell">—</span>
          </template>
        </el-table-column>
...
async function onSync() {
  ...
  if (result.ok) {
    ElMessage.success(result.output || 'Synced')
  } else {
...
const scatterOption = computed(() => {
  ...
  const data = raw.map(e => ({
    ...
    itemStyle: {
      ...
      opacity: 0.75,
      borderColor: providerColor(e.provider),
      borderWidth: 1.5,
  ...
  return {
    tooltip: {
      backgroundColor: c.scatterTooltipBg.value,
      borderColor: c.tooltipBorder.value,
      borderRadius: 14,
      padding: [12, 16],
      textStyle: { color: c.tooltipText.value, fontSize: 13, fontFamily: 'var(--font-family, inherit)' },
      formatter: (params: any) => {
        const d = params.data.value || params.data
        return `<div>
          <strong style="font-size:14px;color:${c.tooltipText.value}">${d[3]}</strong>
          <div style="margin-top:6px;color:${c.tooltipTextSecondary.value};font-size:12px">${d[4]}</div>
          <div style="margin-top:8px;display:flex;flex-direction:column;gap:2px;color:${c.tooltipText.value}">
            <span style="color:var(--color-accent)">Score: <b>${d[0]}%</b></span>
    ...
    grid: { left: 65, right: 40, top: 40, bottom: 55 },
    xAxis: {
      name: t.leaderboard?.overallScore || 'Overall Score',
      nameTextStyle: { fontSize: 12, color: c.axisLabel.value, fontWeight: 600 },
      axisLabel: { color: c.axisLabel.value, formatter: '{value}%' },
      splitLine: { lineStyle: { color: c.scatterGridLine.value } },
      ...markLineAt(medianScore, 'xAxis', 'median'),
    },
    yAxis: {
      name: 'Speed (t/s)',
      nameTextStyle: { fontSize: 12, color: c.axisLabel.value, fontWeight: 600 },
      axisLabel: { color: c.axisLabel.value },
      splitLine: { lineStyle: { color: c.scatterGridLine.value } },
    ...
    series: [
      {
        ...
        emphasis: {
          scale: 1.6,
          focus: 'self' as const,
          label: { show: true, formatter: (p: any) => p.data.value?.[3], position: 'top', fontSize: 12, fontWeight: 600 },
        },
      },
      {
        type: 'line',
        ...
        lineStyle: { color: '#e0a030', type: 'dashed', width: 2, opacity: 0.7 },
        itemStyle: { color: '#e0a030', opacity: 0 },
        symbol: 'diamond',
        symbolSize: 6,
        silent: true,
        z: 1,
  ...
  return {
    [axis]: {
      markLine: {
        silent: true,
        symbol: 'none',
        lineStyle: { color: c.scatterMarkLine.value, type: 'dashed', width: 1, opacity: 0.6 },
        label: { show: false },
        data: [{ [axis]: value }],
      },
    },
