...
def sync_leaderboard(
    ...
    try:
        ...
        if result.returncode == 0:
            # Extract the count from output if available
            output = result.stdout.strip()
            # Look for "Done: N models" or similar
            import re
            m = re.search(r'(\d+)\s*models?\s*upserted', output)
            count = int(m.group(1)) if m else 0
            # Auto-populate tags for models that don't have them yet
