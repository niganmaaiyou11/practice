...
MODELS_REGISTRY_PATH = Path(__file__).resolve().parent.parent / "data" / "models.json"
PROVIDER_ALIASES = {
...
def _norm_key(value: str | None) -> str:
    ...
    return PROVIDER_ALIASES.get((value or "").strip().lower(), key)


def _price_map(db: Session | None = None) -> dict[tuple[str, str], tuple[float, float]]:
    ...
    for provider in registry.get("providers", []):
        provider_keys = {_norm_key(provider.get("id")), _norm_key(provider.get("name"))}
        for model in provider.get("models", []):
            ...
            if pricing.get("input") is None or pricing.get("output") is None:
                continue
            model_key = _norm_key(model.get("name"))
            for provider_key in provider_keys:
                if provider_key:
                    prices[(provider_key, model_key)] = (float(pricing["input"]), float(pricing["output"]))
    return prices


def _costs(prices: dict[tuple[str, str], tuple[float, float]], row: dict) -> tuple[float, float, float]:
    price_input, price_output = prices.get(
        (_norm_key(row["provider"]), _norm_key(row["model_name"])),
        (0.0, 0.0),
    )
    input_cost = (row["total_input_tokens"] or 0) * price_input / 1_000_000
    output_cost = (row["total_output_tokens"] or 0) * price_output / 1_000_000
    return input_cost, output_cost, input_cost + output_cost


def _usage_by_model_query(db: Session, start_date, end_date, provider):
    ...
    return query.group_by(TokenUsage.date, TokenUsage.model_name, TokenUsage.provider)
...
def get_daily_summary(
    ...
    rows = _usage_by_model_query(db, start_date, end_date, provider).order_by(TokenUsage.date.asc()).all()
    ...
    for row in rows:
        ...
        input_cost, output_cost, total_cost = _costs(prices, item)
        current["total_input_tokens"] += item["total_input_tokens"] or 0
        current["total_output_tokens"] += item["total_output_tokens"] or 0
        current["total_tokens"] += item["total_tokens"] or 0
        current["record_count"] += item["record_count"] or 0
        current["total_input_cost"] += input_cost
        current["total_output_cost"] += output_cost
...
def get_model_breakdown(
    ...
    if f is not None:
        query = query.filter(f)
    rows = query.group_by(TokenUsage.model_name, TokenUsage.provider).all()
    result = []
    for row in rows:
        item = dict(row._mapping)
        input_cost, output_cost, total_cost = _costs(prices, item)
        item["total_input_cost"] = input_cost
        item["total_output_cost"] = output_cost
...
def get_provider_breakdown(
    ...
    prices = _price_map(db)
    rows = _usage_by_model_query(db, start_date, end_date, provider).all()
    ...
    for row in rows:
        ...
        current["total_output_cost"] += output_cost
        current["total_cost"] += total_cost
    return sorted(by_provider.values(), key=lambda row: row["total_tokens"], reverse=True)
...
def get_totals(
    ...
    for row in rows:
        item = dict(row._mapping)
        input_cost, output_cost, total_cost = _costs(prices, item)
        totals["total_input_tokens"] += item["total_input_tokens"] or 0
        totals["total_output_tokens"] += item["total_output_tokens"] or 0
        totals["total_tokens"] += item["total_tokens"] or 0
        totals["record_count"] += item["record_count"] or 0
        totals["total_input_cost"] += input_cost
        totals["total_output_cost"] += output_cost
...
OPEN_SOURCE_ORGS = {
    ...
    "SenseTime", "LongCat", "Xiaomi", "MiMo", "Muse", "HiDream",
