...
def run_migrations():
    ...
    try:
        ...
        # weekly news items table
        cur.execute("""
            CREATE TABLE IF NOT EXISTS weekly_news_items (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                slug VARCHAR(220) NOT NULL UNIQUE,
                title VARCHAR(500) NOT NULL,
                summary TEXT NOT NULL,
                category VARCHAR(30) NOT NULL,
                impact VARCHAR(20) DEFAULT 'Watch' NOT NULL,
                reading_time VARCHAR(30) DEFAULT '3 min read' NOT NULL,
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_weekly_slug ON weekly_news_items(slug)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_weekly_category_date ON weekly_news_items(category, published_at)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_weekly_published ON weekly_news_items(is_published)")
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_notification_user ON notifications(user_id)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_notification_user_read ON notifications(user_id, is_read)")
