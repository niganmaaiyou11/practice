...
import type { ModelEntry, ModelsRegistry } from '../types'
...
export const useModelsStore = defineStore('models', () => {
  ...
  const modelsByProvider = computed(() => {
    ...
    for (const p of registry.value.providers) {
      map[p.name] = p.models.map(modelName)
    }
    return map
  })
...
function modelName(model: string | ModelEntry): string {
...
function getStaticFallback(): ModelsRegistry {
  return {
    providers: [
      ...
      { id: 'moonshot', name: 'Moonshot (Kimi)', color: '#3d3d3d', models: ['kimi-k2.6', 'kimi-k2.5'] },
      { id: 'xiaomi', name: 'Xiaomi', color: '#ff6900', models: ['mimo-v2.5-pro', 'mimo-v2-pro'] },
      { id: 'muse', name: 'Muse', color: '#7c3aed', models: ['muse-spark'] },
    ],
    updated_at: null,
  }
}
